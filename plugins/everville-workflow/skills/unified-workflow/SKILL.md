...
3. **PROVE** — run targeted local checks. If user-visible and the user has explicitly authorized publishing this branch/PR, open or update a draft PR, obtain its preview, and test that preview before review. Without publication authority, verify locally, establish a candidate commit SHA, obtain independent review against that SHA, and report that preview evidence is pending publication rather than pushing implicitly. A review finding that changes code creates a new candidate SHA and requires affected checks/review again.
4. **FINISH** — prepare the final PR title/body and evidence from the final diff. Update an existing PR only when that remote edit was explicitly authorized. Confirm checks and review belong to the final candidate SHA. Merge only when the user explicitly requests or authorizes that exact merge; otherwise stop at a release-ready local branch or reviewed PR and report the next external action.
...
1. Confirm the user explicitly authorized pushing this branch and creating or updating the draft PR. If not, stop at a release-ready local candidate and report that preview/E2E is pending publication.
2. Push the current commit and open/update the authorized draft PR.
3. Generate the PR body with `/everville-workflow:explain-pr-changes` when available, or write the same evidence manually.
4. Wait for the preview deployment associated with that commit.
5. Run the project’s E2E suite against the preview URL, using `everville-e2e-discipline:e2e-discipline` when installed or the repository’s existing Playwright convention.
...
Tier 1 additionally needs security and deploy review when those concerns apply. New or modified `SKILL.md` files require `everville-meta:everville-skill-judge` before merge when the maintainer plugin is installed; otherwise perform the same rubric-driven cold review locally and record that fallback.
...
Prepare the PR body from the final diff. Publish that body only with authorization to edit the target PR. Inspect existing CI read-only through the repository’s native checks; invoke `loop-on-ci` only when the user explicitly requests that side-effecting CI repair procedure. Before an authorized merge, prove the PR head SHA, preview/E2E evidence, required review, and required checks all correspond to the current local HEAD. Never infer push, PR creation/edit, or merge authority from an ordinary implementation request.
...
Common optional helpers include Superpowers planning/TDD/worktree/review/verification skills, Beads, codebase-pattern-finder, E2E discipline, loop-on-ci, explain-pr-changes, and the `everville-meta` skill-quality tools.
