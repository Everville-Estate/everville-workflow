...
description: Evaluate Agent Skill design quality before accepting it into the everville-workflow marketplace. Use whenever a new SKILL.md is proposed, a vendored skill is updated, or an existing skill is audited. Scores nine dimensions (120 pts core + 10 pts Everville Fit = 130) and emits actionable improvements.
---
...
<!--
  ...
  Everville modifications: renamed to everville-skill-judge; added D9 "Everville Fit"
  dimension (+10 points, max 130); description requires invocation before any new
  skill is accepted into the everville-workflow marketplace.
-->

# Everville Skill Judge

Evaluate Agent Skills against official specifications and patterns derived from 17+ official examples, plus Everville-specific fit criteria.
...
## Evaluation Dimensions (130 points total — 9 dimensions)
...
### D9: Everville Fit (10 points) — MARKETPLACE GATE

The Everville-specific dimension. A skill can be world-class in isolation and still be wrong for this marketplace. Check each criterion:

| Points | Criterion |
|--------|-----------|
| 2 | **Stack alignment.** Code/examples match Next.js 14+ App Router, TypeScript, Supabase (Postgres + Auth + Storage + Edge Functions), Tailwind, shadcn/ui, Drizzle, Vercel. No examples in stacks we don't run (Rails, Django, raw SQL ORMs, jQuery) unless the skill is stack-agnostic on purpose. |
| 2 | **No unvetted third-party dependencies.** Skill does not require installing CLIs, MCP servers, or binaries we haven't already approved. Uses tools already present in an Everville repo: `gh`, `git`, `pnpm`, `npx`, `psql`, `beads`, `playwright`, `vercel` (API only), `op`. |
| 2 | **No redundant persona framing.** No "You are an expert 10x engineer" / "You are Code-Sage" / emoji preambles. Claude is already Claude — the skill adds knowledge, not costume. |
| 2 | **Doesn't duplicate an installed superpowers skill.** Not a re-skin of `superpowers:brainstorming`, `writing-plans`, `executing-plans`, `test-driven-development`, `systematic-debugging`, `requesting-code-review`, `using-git-worktrees`, or `finishing-a-development-branch`. If it overlaps, it must clearly compose with them, not replace them. |
| 2 | **Respects Everville rules from global CLAUDE.md.** Does not tell Claude to `echo` env vars, raw-SQL-insert into `auth.users`, use the Vercel CLI for Everville deploys, or ask for API keys. Does not instruct sub-selects of the same table in its own RLS policy. Flags rather than enforces behavior that conflicts with stored feedback memories. |

**Scoring band**:
| Score | Meaning |
|-------|---------|
| 0-3 | Wrong marketplace — reject or send back for adaptation |
| 4-6 | Fixable fit issues — list them as blockers |
| 7-8 | Minor friction — ship but open a cleanup ticket |
| 9-10 | Native to the Everville stack |

**Good fit** (from everville-reduce-entropy):
```markdown
Bias toward deletion. Before adding a module, ask if an existing
Drizzle schema, Supabase RPC, or shadcn/ui primitive already covers it.
```

**Bad fit** (from a rejected candidate):
```markdown
You are an expert TypeORM developer. First install @nestjs/cli globally...
```
...
Max = 130 points  (120 core + 10 Everville Fit)
```

**Grade Scale** (percentage-based, out of 130):
| Grade | Percentage | Threshold | Meaning |
|-------|------------|-----------|---------|
| A | 90%+ | 117+ | Excellent — accept into marketplace |
| B | 80-89% | 104-116 | Good — accept with minor fixes |
| C | 70-79% | 91-103 | Adequate — fix blockers before accepting |
| D | 60-69% | 78-90 | Below Average — significant rework |
| F | <60% | <78 | Reject — fundamental redesign needed |

**Hard gate**: any skill scoring <4 on D9 (Everville Fit) is rejected regardless of total score.
...
- **Everville Fit**: [Native / Adaptable / Reject]
