...
| --- | --- | --- |
| `everville-workflow` | A two-track development workflow, review/audit skills, CI helpers, two agents, `/explain-pr-changes`, and workflow hooks | A `SessionStart` hook may add workflow context in an Everville repository. A `PreToolUse` hook checks matched mutation-capable calls. Some skills call `git`, `gh`, or Claude subprocesses when invoked. |
...
| `everville-meta` | Marketplace maintenance and plugin authoring guidance | May scaffold or modify plugin files when explicitly used. Publishing remains a separate, reviewed Git operation. |
...
- **TRIVIAL**: a narrow hard-list skip such as a typo-only change. No ritual is required.
...
- `git`
- `python3` for the shipped hook and compliance harness
...
- PyYAML for `everville-skill-comply`: `uv pip install pyyaml` or `python3 -m pip install pyyaml`
