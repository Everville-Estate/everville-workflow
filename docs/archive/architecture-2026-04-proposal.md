# Historical proposal: Everville Workflow architecture (April 2026)

> **Archived — not current documentation.** This draft describes components, dependencies, repository privacy, and operating decisions that were proposed in April 2026 but were not all shipped. Do not use it for installation, runtime contracts, or release procedures. See [`../architecture.md`](../architecture.md) for the current architecture.
...
| Q9 | Distribution | Restricted-access repository in `Everville-Estate` org, multi-plugin marketplace (proposal only) |
